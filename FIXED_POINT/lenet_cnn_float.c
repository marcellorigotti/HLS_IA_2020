...
void lenet_cnn(	short 	input[IMG_DEPTH][IMG_HEIGHT][IMG_WIDTH], 							// IN
  ...
  // WritePgmFile(output_filename, (float *)CONV1_NBOUTPUT[0], CONV1_WIDTH, CONV1_HEIGHT);
  // printf("\nConv1 output[0]: \n");
  // for (y=0; y<CONV1_HEIGHT; y++) {
  //   for (x=0; x<CONV1_WIDTH; x++)
  //     printf("%d ", conv1_output[0][y][x]);
  //   printf("\n");
  // }
...
short 			INPUT_NORM[IMG_DEPTH][IMG_HEIGHT][IMG_WIDTH];
...
//    printf("%s\n", img_filename);
    ...
    NormalizeImg((unsigned char *)REF_IMG, (short *)INPUT_NORM, IMG_WIDTH, IMG_WIDTH);
